...
void draw_window(Window* win, int focused) {
    ...
    for (int y = 0; y < win->height; y++) {
        for (int x = 0; x < win->width; x++) {
            draw_pixel(win->x + x, win->y + y, fill_color);
        }
    }
...
void render_windows() {
    for (int i = 0; i < window_count; i++) {
        draw_window(&windows[i], (i == focused_window));
    }
}
...
int event_check() {
    ...
    if (mod_down) {
        ...
        //fix this
        else if (w) focused_window = (focused_window == 0) ? window_count - 1 : focused_window - 1;
        else if (s) focused_window = (focused_window + 1) % window_count;
