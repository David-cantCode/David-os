...
void PIC_sendEOI(uint8_t irq) //END OF INTERUPT
{
	if(irq >= 8)
...
//************************************************************/
//**********IF YOUR GOING TO USE APIC and/ or IOAPIC *******/
//************************************************************/
...

{
    ...
    outb(PIC1_DATA, 1 << CASCADE_IRQ); io_wait();
    outb(PIC2_DATA, 2); io_wait();
    outb(PIC1_DATA, ICW4_8086); io_wait();
    outb(PIC2_DATA, ICW4_8086); io_wait();


    //so far no stack errors

    
    // Optionally mask all IRQs here
    outb(PIC1_DATA, 0xFF);
    outb(PIC2_DATA, 0xFF);
